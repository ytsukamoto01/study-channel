...
export function parseListParams(req) {
  ...
  return {
    ...
    order: (url.searchParams.get('order') || 'desc').toLowerCase(),
    // キーセット用カーソル（created_at, id）
    cursorTs: url.searchParams.get('cursor_ts') || null, // ISO文字列
    cursorId: url.searchParams.get('cursor_id') || null, // uuid
    // 任意の追加フィルタ（例：カテゴリ）
    category: url.searchParams.get('category') || null,
    threadId: url.searchParams.get('thread_id') || null,
    parentId: url.searchParams.get('parent_id') || null,
  };
}
